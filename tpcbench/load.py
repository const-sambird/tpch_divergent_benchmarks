...
def clean_database(conn, tables):
...
def create_schema(query_root, conn, db_name, prep_query_dir):
...
def load_tables(data_dir, conn, db_name, tables, load_dir):
...
def index_tables(query_root, conn, db_name, prep_query_dir):
